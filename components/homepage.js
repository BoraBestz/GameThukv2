...
function homepage() {
  ...
  const [gamesmediaList, setGamesmediaList] = useState([]);
  const getGamesmedia = () => {
    Axios.get("http://localhost:3001/games_media").then((response) => {
      setGamesmediaList(response.data);
    });
  };
